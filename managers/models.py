...
class Manager(models.Model):
    """
    Manager model — a sub-account created under an Organisation.
    Replaces the old Assistant / client_staff role.

    Role: 'manager' (was 'client_staff' / 'assistant')
    """
    ...
    # Tables this manager can access (empty = all tables in the organisation)
    assigned_tables = models.ManyToManyField(
        'tables.Table',
        blank=True,
        related_name='assigned_managers',
        help_text='Tables this manager can manage. Empty = all tables in the organisation.',
    )
    assigned_table_ids = models.JSONField(
        default=list,
        blank=True,
        help_text='Optional legacy table IDs this manager can access.',
    )
    ...
    perm_manage_managers = models.BooleanField(default=False)
    ...
    def perm_manage_client_staff(self):
        return self.perm_manage_managers

    @perm_manage_client_staff.setter
    def perm_manage_client_staff(self, value):
        self.perm_manage_managers = value

    class Meta:
        ordering = ['-created_at']

    def __str__(self):
        return f"{self.user.get_full_name() or self.user.username} — Manager"
